...
> **Migration note (2026-09):** The delivery architecture in this document is superseded by the Light SDK sandbox: tool code cannot declare a `Service`, a `BroadcastReceiver`, or send broadcasts, and the SDK's own `LightPushService` owns the UnifiedPush connector, delivering `ByteArray` payloads to `@EntryPoint.onPushNotification` (processed by `push/PushProcessor`). The JNI wrapper and `RustPushDistributorService` sketched below were never built, and the pre-migration `ipc/RustpushService` was dropped. What survives intact: the APNs-in-rustpush rationale, the "endpoint problem" analysis (§ further down), the trade-offs, and the Rust-side interface expectations — those still describe what the future `native-service` bridge must do. The Kotlin contract for that bridge is now length-prefixed JSON over `/dev/socket/rustpush_ipc` (see design.md §2.1 and ADR-005's migration amendment). Post-migration, bundled native libraries are allowlisted by the SDK plugin, and the `rustpush` submodule + `native-service` module shell are in the repo — but no cross-compile or packaging is wired yet.
