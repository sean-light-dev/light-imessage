...
> **Migration note (2026-09):** The domain core of this milestone migrated and is unit-tested (121 passing tests): `AuthStateMachine` (retry guards: 3 login attempts with 1s/2s/4s backoff, 3 2FA attempts, 3 resends, refresh with 1s→60s cap) behind the `AuthManager` facade (`IAuthManager`) — the spec's `AppleIdAuth`. Everything UI- and lifecycle-facing is **pending**: no `AuthViewModel`, no auth screens (only an auth-state banner on `ConversationListScreen`), no session resume on launch, no proactive 300s-window refresh, and nothing in the tool calls `startAuthentication` yet. `SessionRepository` is `EncryptedTokenRepository` (DataStore + AndroidKeyStore AES-256-GCM; see ADR-006 amendment) — no Room identity tables, no `AuthAttempt` audit trail. `HardwareInfoCollector` does not exist (the sandbox bans the `Build.*` surface it would need); provisioning runs over HTTPS via `ProvisioningHttpClient`, not through `NativeServiceClient` `ACTIVATE`. Endpoint paths also drifted pre-migration: the code posts to `/relay/login`, `/relay/verify-2fa`, `/relay/resend-2fa`, `/relay/refresh-token`, `/provisioning/register-hardware`, `/provisioning/activation-status` — the `/api/v1/*` paths below were never implemented. Component status: [../migration-status.md](../migration-status.md).
...
- Attachment upload/download pipeline. *(Correction: no current milestone owns this — Milestone 5 is the Messaging Service and scopes it out; tracked as follow-up work.)*
