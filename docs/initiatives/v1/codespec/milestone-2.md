...
> **Migration note (2026-09):** This spec's Kotlin was migrated into the `light-imessage` SDK tool (`com.thelightphone.lightimessage`). Most of it landed as-written (codecs, crypto, auth state machine, relay service, Room layer); three components were **adapted to the sandbox**: `PushReceiver`/`PushProcessingWorker` → `push/PushProcessor` driven by `@EntryPoint.onPushNotification` (SDK owns UnifiedPush); `BackgroundSyncWorker` → `@LightJob` via `LightWork` (constraints/backoff config dropped); `EncryptedTokenRepository` → DataStore + AndroidKeyStore AES-256-GCM instead of EncryptedSharedPreferences. Component-level status: [../migration-status.md](../migration-status.md). Sections marked with such notes supersede the historical text beneath them.
...
- `BackgroundSyncWorker` using `androidx.work`. *(Migrated: `sync/BackgroundSyncJob.kt` `@LightJob` — see ADR-008 amendment.)*
- `PushReceiver` consuming `org.unifiedpush.android:connector` events. *(Migrated: SDK entry point → `push/PushProcessor` — see ADR-005 amendment.)*

**Scope Out:**

- UI screens, ViewModels, and Compose navigation (Milestone 6). *(Correction: this said "Milestone 3"; UI is proposal phase 6. Two read-only screens landed early during migration anyway.)*
- Full attachment upload/download orchestration beyond metadata persistence. *(Correction: no current milestone owns this — Milestone 5 is the Messaging Service and scopes it out; tracked as follow-up work.)*
...
- Only dependencies in the `ALLOWED_DEPENDENCIES` allowlist in `plugin/src/main/kotlin/com/thelightphone/plugin/LightSdkPlugin.kt` are permitted [^2].
...
classDiagram
    ...
    class AuthManager {
        ...
        - relayClient: IRelayHttpClient
        - nativeClient: IProvisioningClient
...
> **Migration note:** `ThreadSynchronizer` never existed in code (old or new) — the thread upsert is performed inline by the persisting component (`PushProcessor` today). The `PR->>RS: requestSync()` edge is also dead: the migrated push path persists directly and does not trigger a relay sync.
...
**Postconditions:** `SessionTokenEntity` persisted; `AuthState` is `Activated`; push token registered with rustpush. *(Migration note: no `RegisterPushToken` IPC command exists; push registration is SDK-owned (`enablePushNotifications = true`), and forwarding push credentials to the relay is a TODO in `ImessageEntryPoint`.)*

### 4.4 Native Push Notification

> **Migration note:** This sequence was replaced in the SDK tool. The SDK's `LightPushService` owns the UnifiedPush receiver; payloads arrive at `ImessageEntryPoint.onPushNotification(ByteArray)` and are processed inline by `push/PushProcessor` (parse → existence-dedup → decode envelope → persist message + thread in one Room transaction). There is no `PushReceiver` broadcast, no per-push WorkManager hop, and the push DTO has no `type` field (no WAKE / READ_RECEIPT / TYPING routing — only message delivery is handled). Pushes arriving before `AppServices` is initialized (cold process) are logged and recovered by the next periodic sync instead of being queued.
...
> **Migration note:** The flow below survives, but the executor changed: a top-level `@LightJob` handler (`sync/BackgroundSyncJob.kt`) scheduled via `LightWork.enqueuePeriodic(lightContext, "background-sync", 15.minutes)` from `ConversationListScreen.willShow()` (ADR-008 amendment). `LightWork` exposes no constraint API, so the old `NetworkType.CONNECTED` + battery-not-low constraints and the custom exponential backoff were dropped; `LightJobResult.Retry` yields WorkManager's default backoff. Dependencies arrive via the `AppServices` locator, not a `WorkerFactory`.
...
> **Migration note:** The migrated unit suite (121 tests, `kotlin.test` + hand-written fakes) covers §7.1's auth/codec/plist/crypto rows. The integration rows below were **not** migrated: `androidx.test` (androidTest) and `okhttp3:mockwebserver` (`RelayServiceIntegrationTest`) are not on the SDK dependency allowlist. Reinstating them needs either an allowlist change or a fake-socket/MockServer-free rewrite.
...
> **Migration note:** This section is a pre-migration branch snapshot kept for history. It is superseded by [../migration-status.md](../migration-status.md): F-1 (`ImeApplication`/`AppWorkerFactory`), F-5 (worker DI), F-9/F-12 (androidTest) and the TASK_009/TASK_011 components changed shape or were dropped under the SDK sandbox.
