...
> **Migration note (2026-09):** This is the milestone most reshaped by the SDK sandbox. `PushReceiver` does not exist (SDK `LightPushService` → `ImessageEntryPoint.onPushNotification` → `push/PushProcessor`); `PushHandler` exists only as the delivery path inside `PushProcessor` (no WAKE/READ_RECEIPT/TYPING routing — the push DTO is untyped); per-push WorkManager is gone (a 15-minute `LightWork` periodic job is the only deferred sync); `NativeServiceClient` migrated intact (framing, 30s heartbeat / 5s pong timeout, ≤100 queue, correlation IDs) but is constructed nowhere — there is no native process to connect to (`native-service/` is a Gradle module shell; cargo cross-compile and `.so` packaging unwired). Also unresolved from before migration: the Kotlin client speaks `{"command":"ping"}` frames while the Rust `protocol.rs` expects serde-tagged `PING`/`Pong` events — the two wire contracts must be reconciled when the Rust build is wired. Component status: [../migration-status.md](../migration-status.md).
...
- Kotlin `PushReceiver` registered with `org.unifiedpush.android:connector`. *(Superseded: SDK owns the receiver — see ADR-005 amendment.)*
- `PushHandler` domain service routing push types to sync, message insertion, read receipt updates, or typing indicators. *(Partially delivered: `PushProcessor` covers message insertion only; push payloads carry no `type` field.)*
...
- `WorkManager` integration for deferred sync when push is received while the app is not in the foreground. *(Adapted: periodic-only `@LightJob` via `LightWork`; no per-push expedited sync — see ADR-008 amendment.)*
...
- UI screens for conversation list or thread detail (Milestone 6). *(Correction: this said "Milestone 4"; UI is proposal phase 6.)*
- Full attachment download/upload pipeline. *(Correction: no current milestone owns this — Milestone 5 is the Messaging Service and scopes it out; tracked as follow-up work.)*
...
| MESSAGE_ENTITY         | status                 | int     | NOT NULL         | `0=DRAFT`, `1=ENCRYPTED`, `2=SENT`, `3=DELIVERED`, `4=READ`, `5=FAILED`. *(Corrected 2026-09: was `1=SUBMITTED`; the code has never had a SUBMITTED constant — `PushProcessor` defines `STATUS_ENCRYPTED = 1`.)* |
...
> **Migration note:** Pending. The `native-service/` module in `light-imessage` is a Gradle shell (Rust sources + lockfile; no cargo cross-compile, no `.so` in the APK), and `android.app.Service` is banned in the sandbox, so the launch flow below cannot run in-process. The bridge must be hosted outside the tool (OS-level socket at `/dev/socket/rustpush_ipc`) or via a future SDK capability.
...
> **Migration note:** Two deviations from the state machine below. (1) The Kotlin client reconnects on a **single** 5s pong timeout, not 3 missed PONGs (this spec is internally inconsistent — the invariant in §1 says one miss means lost; the machine and test matrix say 3). (2) The implemented states are `Disconnected / Connecting / Connected / Reconnecting(attempt, nextRetryIn) / Failed(error)` — there is no launcher, so no `NotRunning`/`Starting` states, and reconnect is capped at 5 attempts with 1s→32s backoff plus jitter before terminal `Failed`.
...
> **Migration note:** Task/deliverable paths in this section are pre-migration. Current reality per component: [../migration-status.md](../migration-status.md). Notable remap: `domain/push/PushReceiver.kt` → `push/PushProcessor.kt`; `domain/sync/BackgroundSyncWorker.kt` → `sync/BackgroundSyncJob.kt`; `domain/auth/AuthService.kt` → `domain/auth/AuthManager.kt`; heartbeat lives inside `NativeServiceClient` (no separate `HeartbeatManager`); the `androidTest` deliverable was not migrated.
