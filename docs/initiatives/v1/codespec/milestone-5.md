...
> **Migration note (2026-09):** Roughly the persistence-and-transport half of this milestone migrated: `MessageRepository`/`ThreadRepository`/`ContactRepository` (Room + Flow), `RelayService` (WebSocket send/receive, keepalive, reconnect), and `MessageCodec`/`PlistCodec`/`CryptoEngine` — note the codec is the Apple Plist/AES-GCM envelope in **Kotlin**, not the IPC JSON codec this spec assumes (and not inside rustpush, contra §Scope Out). **Pending:** `MessagingService` orchestration, drafts, attachments, read receipts, typing indicators, composer, `ThreadSynchronizer`, `MessageStatusUpdater`, and `sinceToken`-based sync (`requestSync()` takes no token). Two spec/code mismatches to resolve when implementing: thread IDs are `UUID.nameUUIDFromBytes` (MD5-based) over sorted participant URIs, not SHA-256 truncated to 16 bytes (§6.1); and there is **no duplicate-send detection window** — outbound sends have no idempotency guard at all (inbound dedup is existence-based). Component status: [../migration-status.md](../migration-status.md).
...
This specification builds on the rustpush-native architecture established in the project proposal and [design.md](../design.md), the IPC/heartbeat infrastructure from Milestone 3, and the auth/session layer from Milestone 4.
