...
The codebase migrated from the standalone `light-message` app into the `light-imessage` SDK tool scaffold. Phase status against the table above (full register: [migration-status.md](./migration-status.md)):

| Phase | Status |
| ----- | ------ |
| 0. SDK audit | **Superseded** — the SDK's answer to native code arrived as the sandbox itself: no `Service`/`BroadcastReceiver`/custom `Application`; bundled native libraries were allowlisted post-migration (`native-service` module + `rustpush` submodule landed, packaging unwired). |
| 1. Protocol spec | **Complete** (the codespec documents in this repo). |
| 2. Core infrastructure | **Complete** — migrated; unit tests pass (121). The `cargo-ndk` cross-compile half is pending (module shell only). |
| 3. rustpush bridge | **In progress** — Kotlin IPC client migrated (unwired); push consumption is now SDK entry-point driven, not a tool-side `PushReceiver`; native packaging pending. |
| 4. Auth & session | **Code complete, unwired** — `AuthManager`/`AuthStateMachine` migrated with tests; no auth UI (`AuthViewModel`/screens pending), nothing calls `startAuthentication` yet. `sinceToken` persistence not implemented. |
| 5. Messaging service | **Partial** — codec + relay service + repositories migrated; no composer/send UI, no duplicate-send window, no receipts/typing services. |
| 6. UI & keyboard | **Started early** — `ConversationListScreen` + read-only `ThreadScreen` landed during migration; keyboard/composer/settings pending. |
| 7–8 | **Pending.** |
