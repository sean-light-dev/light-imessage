...
> **Migration note (2026-09):** This document predates the migration into the Light SDK tool sandbox (`com.thelightphone.lightimessage`). What changed underneath it: (1) the relay was **not** eliminated — `RelayService` (OkHttp WebSocket) is the message transport today and rustpush-direct APNs is unrealized; (2) the Plist/AES-GCM envelope codec lives in **Kotlin** (`domain/codec`, `domain/crypto`), not inside rustpush; (3) push delivery is SDK entry-point driven (`@EntryPoint.onPushNotification` → `PushProcessor`), not a tool-side UnifiedPush receiver; (4) `android.app.Service`/JNI/AIDL are sandbox-banned — the Kotlin IPC client (`NativeServiceClient`) targets an OS-level Unix socket at `/dev/socket/rustpush_ipc`, and the Rust side is a module shell pending cargo wiring; (5) several linked schemas under `../schemas/` and `../specs/` were never committed (dead links retained below, marked). Current component status: [migration-status.md](./migration-status.md).
...
- **Heartbeat**: Kotlin client sends `PING` every 30s; rustpush responds with `PONG` within 5s. Missing `PONG` within 5s triggers `NativeServiceConnectionLost` and reconnect with exponential backoff (1s, 2s, 4s, 8s, cap 60s). No max retry count; reconnect indefinitely until successful. *(Implemented differently: a single 5s pong timeout triggers reconnect with backoff 1s→32s cap + jitter, max 5 attempts before terminal `NativeServiceState.Failed`.)*
- **Concurrency**: Single IPC connection; commands serialized with queue depth ≤ 100. If queue overflows, client returns `ERROR` with code `IPC_QUEUE_FULL`. rustpush processes asynchronously and returns responses with matching `correlation_id`. *(Implemented as `Result.failure(IOException("IPC queue full"))` on the Kotlin side.)*
...
| `ThreadId`         | UUID name-based (MD5) hash of sorted, `|`-joined participant URI strings (`UUID.nameUUIDFromBytes`) *(corrected 2026-09: spec said SHA-256 truncated to 16 bytes; the code has always used MD5-UUID)* | Deterministic, immutable; collisions documented in code (rare; switch to full 32-byte hash if detected) |
...
> [JSONSchema](../schemas/entityModel.json) *(dead link — schema never committed; the Room entities in `data/entity/` are the de-facto model, and they drifted from §3.5: no `PARTICIPANT` table (pipe-separated `participantUris` column), no `APPLE_ACCOUNT`/`NATIVE_SERVICE_SESSION` Room tables (DataStore instead), `DomainEventEntity` undocumented here)*
...
> **Migration note:** The transition topology below matches the migrated `AuthStateMachine`, but state names drifted: `CredentialsRequired`→`AwaitingCredentials`, `CredentialsSubmitted`→`Authenticating`, `TwoFactorRequired`→`AwaitingTwoFactorCode`, `ActivationInProgress`→`ProvisioningHardware`, `Activated`→`SessionEstablished`, `AuthenticationFailed`→`Failed`, `Deactivated`→`LoggingOut`→`Idle`. There is no `SessionRefreshing` (refresh folds into `SessionEstablished → ProvisioningHardware`) and no explicit `TwoFactorResend` state. The max-3 credential/2FA counters match.
...
> [**JsonSchema: AttachmentRef (IPC Payload)**](../schemas/attachmentRef.json) *(dead link — schema never committed)*
...
[**JsonSchema: Outer Envelope (rustpush internal)**](../schemas/encryptedEnvelope.json) *(dead link — schema never committed. Also "rustpush internal" is wrong post-migration: the Kotlin `MessageCodec` produces/parses exactly this format — see §8 tables, which do match the code)*
...
[**JsonSchema: Inner Payload (rustpush internal)**](../schemas/messagePayload.json) *(dead link — schema never committed; see note above)*
...
[AsyncAPI Spec](../specs/apns-unifiedpush.asyncapi.yaml) *(dead link — spec never committed)*

---

### 10. Mac Relay One-Time Activation API (OpenAPI 3.2.0)

[OpenAPI Spec](../specs/mac-relay-activation.openapi.yaml) *(dead link — spec never committed; the implemented endpoints are `/relay/login`, `/relay/verify-2fa`, `/relay/resend-2fa`, `/relay/refresh-token`, `/provisioning/register-hardware`, `/provisioning/activation-status` — the `/api/v1/*` scheme referenced across these docs was never implemented)*
...
> **Migration note (2026-09):** This gate table is historical. Current status of the flip-prone rows: *NDK `.so` loading* — banned at migration time, allowlisted afterwards (`io.github.david-allison:rust-push-android`), but nothing ships yet (module shell only). *UnifiedPush connector* — consumed by the SDK, not the tool (entry-point push delivery). *IPC framing unit tests* — `IpcFramingTest` migrated as stub TODOs; framing logic is exercised indirectly. *AES-GCM decryption* — verified for the envelope codec (`MessageCodecTest`/`CryptoEngineTest` pass); **not** exercised for attachments (stub test). *Path A* — contradicted in one direction: envelope crypto moved INTO Kotlin rather than staying inside rustpush.
